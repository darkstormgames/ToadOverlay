 ...
 */
const fs = require('fs');
const {workingdirectory} = require('../config.json');
    ...
    */
    logMessage: (message, user = null, guild = null, channel = null) => {
        ...
        if (guild == null && user == null) {
            ...
            fs.appendFile(workingdirectory + 'bot.log', datetime + message + '\n', (err) => {
                if (err) console.log(err);
            });
        }
        else if (guild == null && user != null) {
            console.log(datetime + '[USER: ' + user.username + ' (' + user.id + ')] ' + message);
            fs.appendFile(workingdirectory + 'bot.log', datetime + '[USER: ' + user.username + ' (' + user.id + ')] ' + message + '\n', (err) => {
                if (err) console.log(err);
            });
        }
            ...
            fs.appendFile(workingdirectory + 'bot.log', datetime + '[USER: ' + user.username + ' (' + user.id + ')] [GUILD: ' + guild.name + ' (' + guild.id + ')] ' + message + '\n', (err) => {
                if (err) console.log(err);
            });
        }
        else {
            console.log(datetime + '[USER: ' + user.username + ' (' + user.id + ')] [GUILD: ' + guild.name + ' (' + guild.id + ')] [CHANNEL: ' + channel.name + ' (' + channel.id + ')] ' + message);
            fs.appendFile(workingdirectory + 'bot.log', datetime + '[USER: ' + user.username + ' (' + user.id + ')] [GUILD: ' + guild.name + ' (' + guild.id + ')] [CHANNEL: ' + channel.name + ' (' + channel.id + ')] ' + message + '\n', (err) => {
                if (err) console.log(err);
            });
        }
