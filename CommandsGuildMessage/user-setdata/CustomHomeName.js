    ...
    */
    execute: (message, args) => {
        if (args.length > 0) {
            ...
            Data.ExecuteQuery('UPDATE ' + process.env.SQL_NAME + '.channel_data SET home_mkc_url = "", home_name = "' + Data.sql.connection.escape(args.join(' ')).replaceAll("'", "") + '" WHERE channel_id = ' + message.channel.id + ';', 
            (error) => {
